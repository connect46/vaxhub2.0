...
import ConsumptionScForecast from '@/components/ConsumptionForecastResults'; // <-- Import the new component
...
export default function ConsumptionScForecastPage() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    const fetchInitialData = async () => {
      ...
      const forecastQuery = query(
        collection(db, "forecasts_consumption_sc"),
  ...
  const handleSaveForecast = async () => {
    ...
    try {
      ...
      if (loadedForecastId) {
        await setDoc(doc(db, 'forecasts_consumption_sc', loadedForecastId), dataToSave, { merge: true });
      } else {
        const newDocRef = await addDoc(collection(db, 'forecasts_consumption_sc'), dataToSave);
  ...
  return (
    <ProtectedRoute>
      <Container maxWidth="xl">
        <Box sx={{ my: 4 }}>
          <Typography variant="h4" component="h1" gutterBottom>Consumption-Based Forecast (SC)</Typography>
          ...
          <Paper sx={{ p: 3 }}>
            ...
            <ConsumptionScForecast forecastData={forecastResult} onUpdate={handleForecastUpdate} />
          </Paper>
            </>
          )}
        </Box>
